...
// NOTE: Keep your existing utility classes (chip, panel, btn-primary, btn-ghost, badge-*, bg-blob)
// This file preserves your visual design while adding: a11y, mobile menu focus trap, lazy hero,
// API-loaded modules with graceful fallback, analytics hooks, and world-class perf touches.
...
function PriceCard({ name, m, y, features, billing, highlight = false }) {
  ...
  return (
    <div className={`panel p-5 card-animated reveal ${highlight ? "ring-2 ring-slate-200 dark:ring-cyan-300/30" : ""}`}>
      ...
      <a href="#start" className="btn-primary btn-shine mt-4" onClick={(e)=>{e.preventDefault(); document.querySelector('#start')?.scrollIntoView({behavior:'smooth'});}}>Get {name}</a>
    </div>
  );
}
 ...
 * - adds: Select toggle, module-level Start free
 */
  ...
  return (
    <div className="panel p-4 card-animated reveal">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <button
          onClick={() => { setBusy(true); setTimeout(() => setBusy(false), 800); }}
          className="btn-ghost"
          disabled={busy}
          aria-disabled={busy}
        >
        ...
        <button
          onClick={() => { track('start_free_module', { module: app.id }); onStartFree?.([app.id]); }}
...
function useMobileMenu(aOpen) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      if (e.key === "Escape") { (menuRef.current?.dataset?.close === '1') && (menuRef.current.dataset.close = '0'); document.body.style.overflow = prevOverflow; }
      if (e.key !== "Tab") return;
      const fs = focusables();
      if (!fs || fs.length === 0) return;
      const active = document.activeElement;
      if (e.shiftKey && active === fs[0]) { e.preventDefault(); fs[fs.length - 1].focus(); }
      else if (!e.shiftKey && active === fs[fs.length - 1]) { e.preventDefault(); fs[0].focus(); }
    };

    const t = setTimeout(() => first()?.focus(), 10);
    window.addEventListener("keydown", onKey);
    return () => { clearTimeout(t); window.removeEventListener("keydown", onKey); document.body.style.overflow = prevOverflow; };
  }, [aOpen]);
  return menuRef;
}
...
export default function PublicLanding() {
  ...
  const menuRef = useMobileMenu(menuOpen);
  ...
  const filtered = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      list = list.filter((a) => a.name.toLowerCase().includes(s) || a.desc.toLowerCase().includes(s) || a.tags?.some((t) => t.toLowerCase().includes(s)));
    }
  ...
  const goTo = (hash) => {
    ...
    setTimeout(() => { el.focus({ preventScroll: true }); el.removeAttribute("tabindex"); }, 250);
  };
    ...
    <div className="relative min-h-screen">
      ...
      >
        <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-3" aria-label="Primary">
            ...
            <button className="btn-primary btn-shine" onClick={() => { goTo("#apps"); }}>Start free</button>
          </nav>
        ...
        <>
          ...
          <div id="mobile-menu" role="dialog" aria-modal="true" ref={menuRef} className="fixed inset-x-0 top-16 z-50 md:hidden panel p-4 border-t border-slate-200 dark:border-slate-700">
            <nav className="grid gap-3" aria-label="Mobile">
              <button className="chip text-left" onClick={() => { setMenuOpen(false); goTo("#apps"); }}>Apps</button>
              <button className="chip text-left" onClick={() => { setMenuOpen(false); goTo("#features"); }}>Features</button>
              <button className="chip text-left" onClick={() => { setMenuOpen(false); goTo("#pricing"); }}>Pricing</button>
              <button className="btn-primary btn-shine" onClick={() => { setMenuOpen(false); goTo("#apps"); }}>Start free</button>
            </nav>
          </div>
        </>
      )}
      ...
      <section className="max-w-7xl mx-auto px-5 pt-14 pb-8 grid md:grid-cols-2 gap-8 items-center">
        <div className="reveal">
          ...
          <div className="mt-4 flex gap-2">
            <button className="btn-primary btn-shine" onClick={() => { goTo("#apps"); }}>Start free</button>
            <button className="btn-ghost" onClick={() => { goTo("#apps"); }}>Browse apps</button>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-5">
          <div className="marquee">
            <div className="marquee-track">
              {logoRow.map((n) => (<div key={n} className="text-slate-500 dark:text-slate-300 whitespace-nowrap">{n}</div>))}
              {logoRow.map((n) => (<div key={`${n}-2`} className="text-slate-500 dark:text-slate-300 whitespace-nowrap">{n}</div>))}
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="apps" className="scroll-mt-24 max-w-7xl mx-auto px-5 py-10">
        ...
        <div className="grid gap-3 mb-4">
          <div className="flex flex-wrap gap-2 justify-center reveal">
            {CATEGORIES.map((c) => (
              <CategoryPill key={c.key} active={cat === c.key} onClick={() => setCat(c.key)}>{c.label}</CategoryPill>
            ))}
          </div>
        ...
        <div className="flex justify-center gap-2 mb-6 reveal">
          <div className="panel p-1 inline-flex gap-1">
            {["monthly","yearly"].map((v) => (
              <button key={v} onClick={() => setBilling(v)} className={`px-3 py-1.5 rounded-lg ${billing === v ? "btn-primary" : "btn-ghost"}`}>
      ...
      <section id="start" className="scroll-mt-24 text-center py-12 border-t border-slate-200 bg-gradient-to-b from-slate-50 to-white dark:border-slate-700 dark:from-cyan-300/10 dark:to-purple-500/10">
        ...
        <div className="mt-4 flex gap-3 justify-center flex-wrap">
          <button className="btn-primary btn-shine" onClick={() => startSignup()}>Start free{selected.size ? ` (${selected.size})` : ""}</button>
          <button className="btn-ghost" onClick={() => goTo('#apps')}>Pick apps</button>
        </div>
      </section>
